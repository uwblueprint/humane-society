...
const AddPetPage = (): React.ReactElement => {
  ...
  useEffect(() => {
    const subscription = watch((value) => {
      const hasData =
        ...
        value.breed ||
        value.weight ||
        (value.birthdayMonth && value.birthdayMonth !== "--") ||
        (value.birthdayDate && value.birthdayDate !== "--") ||
        (value.birthdayYear && value.birthdayYear !== "--") ||
        (value.sex && value.sex !== "--") ||
        (value.neutered && value.neutered !== "--") ||
        value.safetyInfo ||
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (isValid) {
      ...
      toast({
        title: "Success",
        description: "Pet added successfully (logged to console)",
        status: "success",
        duration: 3000,
        isClosable: true,
      });
  ...
  const handleNextPage = async () => {
    // Only allow a user to progress if they have resolved all errors on page 1
    const isValid = await trigger([
      "name",
      "colourLevel",
      "animalTag",
  ...
  const colourLevelOptions = ["Green", "Yellow", "Orange", "Red", "Blue"];
  ...
  return (
    <>
      ...
      <PopupModal
        ...
        onPrimaryClick={closeQuitModal}
        secondaryButtonText="Quit"
        onSecondaryClick={handleQuitConfirm}
      />
    </>
  );
};
