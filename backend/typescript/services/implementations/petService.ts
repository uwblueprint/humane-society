...
class PetService implements IPetService {
  ...
  async createPet(pet: PetRequestDTO): Promise<PetResponseDTO> {
    ...
    return {
      ...
      photo: newPet?.photo,
      careInfo: {
        id: newPet?.PetCareInfo.id,
        safetyInfo: newPet?.PetCareInfo.safety_info,
        medicalInfo: newPet?.PetCareInfo.medical_info,
        managementInfo: newPet?.PetCareInfo.management_info,
      },
    };
  }
    ...
    async updatePet(id: string, pet: PetRequestDTO): Promise<PetResponseDTO | null> {
        ...
        try {
            ...
            petCareInfoUpdateResult = await PgPetCareInfo.update(
                ...
                { where: { pet_id: id }, include: PgPetCareInfo, returning: true, transaction },
            ); 
