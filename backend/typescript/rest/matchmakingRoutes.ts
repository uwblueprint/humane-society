...
const matchmakingService: IMatchmakingService = new MatchmakingService(userService, petService);
...
matchmakingRouter.get("/users/:userId/pets", async (req, res) => {
    ...
    try {
        const matchingPets = await matchmakingService.getMatchingPetsForUser(userId);
...
matchmakingRouter.get("/pets/:petId/users", async (req, res) => {
   ...
   try {
    const matchingUsers = await matchmakingService.getMatchingUsersForPet(petId);
